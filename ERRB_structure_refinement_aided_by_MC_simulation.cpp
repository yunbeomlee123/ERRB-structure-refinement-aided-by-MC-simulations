...
/*					double v_rb_avr_tot;
					...
					*/
					...
					/*
					if(bond_break&&(repeat_step>200)){
						system("cp new.pdb ./before.pdb");
						BringTerminus();
						MakePDB();
						system("cp new.pdb ./after.pdb");
				...
				//		update = false;
						...
						if((chi2<chi2_crit)&&(penalty_collision==0.0)&&(bond_break==false)){
								...
								out.close();
								log_book = false;
								// If the logbook has been written in here, logbook writing below here will not work.
								...
								//out<<replica_line<<" : chi2 : "<<chi2<<endl;
								sprintf(chi2_corrected, "%06.4f", chi2);
								out<<replica_line<<" : chi2 : "<<chi2_corrected<<endl;
								out.close();
						}//20180701 --> position of this part has been changed
...
//		system("mv new.pdb ./4SDH_modified.pdb");
